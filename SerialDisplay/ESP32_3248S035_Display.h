#ifndef ESP32_3248S035_Display_H
#define ESP32_3248S035_Display_H
...
public:
  LGFX(void)
  {
    ...
    { // Set the display panel control.//表示パネル制御の設定を行います。
      ...
      cfg.memory_width     =   SCREEN_W;  // Maximum width supported by driver IC // ドライバICがサポートしている最大の幅
      cfg.memory_height    =   SCREEN_H;  // Maximum height supported by driver IC // ドライバICがサポートしている最大の高さ
      cfg.panel_width      =   SCREEN_W;  // Actually displayable width // 実際に表示可能な幅
      cfg.panel_height     =   SCREEN_H;  // Actually displayable height // 実際に表示可能な高さ
    ...
    { // Set the backlight control. (Delete if not needed // バックライト制御の設定を行います。（必要なければ削除）
      ...
      cfg.pin_bl = 27;              // Pin number to which the backlight is connected // バックライトが接続されているピン番号
    ...
    { // Set the touch screen control. (Delete if not needed) // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      cfg.x_max      = TOUCH_W;  // Maximum X value (raw value) obtained from the touch screen // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 0;    // Minimum Y value (raw value) obtained from touch screen // タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = TOUCH_H;  // Maximum Y value (raw value) obtained from the touch screen // タッチスクリーンから得られる最大のY値(生の値)
